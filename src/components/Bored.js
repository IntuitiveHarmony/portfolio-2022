...
const Bored = () => {
  ...
  return (
    <>
      ...
      <p>
        Feeling uninspired? <br /> - Use the dropdown to pick a type (optional){" "}
        <br /> - Press the button to generate a random activity <br />- Keep
        pressing the button to make a list <br />
        <br /> Powered by the{" "}
        <a
      ...
      <div className="submitBtn" onClick={() => handleBored()}>
        Get Activity
      </div>
      {bored ? (
        <>
      ...
      <ul>
        ...
        {list.map((item, index) => (
          <li key={index}>
            {index + 1}) id #
            <a
              href={"http://www.boredapi.com/api/activity?key=" + item.key}
              className="pinkLink"
              target="_blank"
              rel="noopener noreferrer"
            >
            ...
            <span className="greenTxt">{item.type}</span>
            <br />
            &nbsp;&nbsp;&nbsp;&nbsp; {item.activity}
            <br />
          </li>
